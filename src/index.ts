...
app.event("reaction_added", async ({ event }) => {
    ...
    ; (async () => {
        ...
        const messageFreq = parseFloat(process.env.NUMEROLOGY_MESSAGE_FREQ || "15") // average minutes between messages given maximum activity
        const newThreadFreq = parseFloat(process.env.NUMEROLOGY_NEW_THREAD_FREQ || "360") // average minutes between new threads
        const joinFreq = parseFloat(process.env.NUMEROLOGY_JOIN_FREQ || "720") // average minutes between users joining
        const leaveFreq = parseFloat(process.env.NUMEROLOGY_LEAVE_FREQ || "720") // average minutes between users leaving
